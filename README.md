...
- **Battery-aware polling.** Listens to UPower's `OnBattery` over DBus (no polling, no `/sys` reads) and slows the refresh rate on battery by a configurable multiplier.
...
└── contents/
    ...
    └── ui/
        ├── main.qml          PlasmoidItem · fetch loop · UPower DBus listener · backoff
